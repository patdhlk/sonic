...
fn diamond_runs_all_vertices_once() {
    ...
    let r = g.vertex(item_with_triggers(
        ...
        move |_| { c0.fetch_add(1, Ordering::SeqCst); Ok(ControlFlow::Continue) },
    ));
    let c1 = Arc::clone(&counts[1]);
    let l = g.vertex(item(move |_| { c1.fetch_add(1, Ordering::SeqCst); Ok(ControlFlow::Continue) }));
    let c2 = Arc::clone(&counts[2]);
    let rt = g.vertex(item(move |_| { c2.fetch_add(1, Ordering::SeqCst); Ok(ControlFlow::Continue) }));
    let c3 = Arc::clone(&counts[3]);
    let m = g.vertex(item(move |_| { c3.fetch_add(1, Ordering::SeqCst); Ok(ControlFlow::Continue) }));
...
fn root_stop_chain_skips_dependents() {
    ...
    let leaf_v = g.vertex(item(move |_| { l.fetch_add(1, Ordering::SeqCst); Ok(ControlFlow::Continue) }));
