...
use sonic_executor::{Channel, Executor, ExecutableItem, Context, ControlFlow, ExecuteResult, ExecutorError, ItemError, Subscriber, TriggerDeclarer};
use std::sync::Arc;
use std::sync::atomic::{AtomicU64, Ordering};

#[derive(Debug, Clone, Copy, ZeroCopySend)]
#[repr(C)]
struct Big { tag: u64, payload: [u8; 1024] }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    exec.add(sonic_executor::item_with_triggers(
        |d| { d.interval(Duration::from_millis(100)); Ok(()) },
        move |_| {
            let n = c.fetch_add(1, Ordering::SeqCst);
            // Zero-copy send via loan: closure constructs Big directly in shm.
            publisher.loan(|slot: &mut MaybeUninit<Big>| {
                slot.write(Big { tag: n, payload: [0xAB; 1024] });
                true
    ...
    struct Consumer { sub: Subscriber<Big> }
    impl ExecutableItem for Consumer {
        ...
        fn execute(&mut self, _ctx: &mut Context<'_>) -> ExecuteResult {
            while let Some(s) = self.sub.take().map_err(|e| -> ItemError { Box::new(e) })? {
                let p = s.payload();
                println!("[consumer] received tag={} (payload[0]={:#x}, len={})",
                    p.tag, p.payload[0], p.payload.len());
            }
            Ok(ControlFlow::Continue)
        }
    }
